...
const unzipRemoteFile = async (config, index) => {
  try {
    const { webDir, saveZip } = config
    const remoteFileName = `${webDir}.zip`

    log(`(${index}) 解压远程文件 ${underline(remoteFileName)}`)

    let delCMD = `&& rm -rf ${remoteFileName}` ;
    if(saveZip) {
      delCMD = ''
    }

    await ssh.execCommand(
      `unzip -o ${remoteFileName} -d ${webDir} ${delCMD}`
    )
...
module.exports = {
  ...
  apply: async (env) => {
    if (checkDeployConfigExists(deployConfigPath)) {
      const config = (await import(deployConfigPath)).default
      ...
      error(
        `${getDeployConfigFileName()} 文件不存，请使用 deploy-cli-service init 命令创建`
      )
      process.exit(1)
    }
  }
